...
const TrainingRequests = () => {
  return (
    <div className="flex flex-col gap-[8px]">
      <div className="text-lg font-bold text-[#413B3B]">Yêu cầu đào tạo</div>
      <div className="grid gap-x-[12px] gap-y-[8px] grid-cols-3">
        {data.map((item, index) => (
          <TrainingRequestsStatisticsCard
            title={item.title}
            icon={item.icon}
...
const TrainingRequestsStatisticsCard: TTrainingRequestsStatisticsCard = ({
  background,
  icon,
  title,
  value,
...
export { TrainingRequests };

type TTrainingRequestsStatisticsCard = FC<{
  background: string;
  icon: string;
  title: string;
  value: number;
